    ...
    pygame.display.update()`,
      ...
      steps: [
        ...
        },
        {
          lines: [19, 20, 22, 23],
          frame: 'Frame 60',
          label: '12 · fast forward: one second of holding Right',
          desc: 'Fifty-eight more trips round the loop, each one calling update and draw on the same object. 60 frames times 4.8 pixels is 300 pixels. The player has crossed from x = 380 to x = 680.',
          delta: () => ({ 'self.rect.x': { from: 389.6, to: 680 } }),
          apply: (s) => ({ ...s, frame: 60, player: { ...s.player, x: 680 } }),
        },
        {
          lines: [9, 10],
          frame: 'Frame 120',
          label: '13 · another second, and the edge',
          desc: 'Line 9 would push x past 760. Line 10 pulls it back every frame, so the box stops flush with the right edge. clamp_ip runs after the move because it corrects the move.',
          delta: () => ({ 'self.rect.x': { from: 680, to: 760 } }),
          apply: (s) => ({ ...s, frame: 120, player: { ...s.player, x: 760 } }),
        },
      ],
    },
  },
